    ...
    foreach ($data as $row) {
        ...
        if (!empty($userData)) {
            $dbh->query("UPDATE statistics_user_mentions
                         SET last_week_mentions = " . $row['count'] . "
                         WHERE user_id = '" . $row['user_id'] . "'");
        } else {
            $dbh->query("INSERT INTO statistics_user_mentions (user_id, screen_name, last_week_mentions)
                         VALUES ('" . $row['user_id'] . "', '" . $row['screen_name'] . "', " . $row['count'] . ")");
        }
    }
}

updateWeekMentionsCount(date('Y-m-d', strtotime(date('Y-m-d') . '-1 day')));
