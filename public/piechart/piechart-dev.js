// DVSL Time Chart library. Development Version.
...
Piechart_Selection = (function() {
  ...
  Piechart_Selection.prototype.updateSelection = function(event, target, softDeselect) {
    ...
    if (!event.shiftKey && (!target || Base_Helpers.arrayContains(this.scene.selection, target))) {
  ...
  Piechart_Selection.prototype.switchCurrentSlice = function(obj, event) {
    ...
    if (this.scene.activeSlice) {
      this.scene.activeSlice.active = false;
    }
    if (this.scene.activePie) {
      this.scene.activePie.active = false;
    }
    this.scene.activeSlice = obj;
    this.scene.activePie = null;
    this.currentSlice = obj;
    this.currentPie = null;
    if (this.currentSlice) {
      this.scene.activeSlice.active = true;
  ...
  Piechart_Selection.prototype.switchCurrentPie = function(obj, event) {
    ...
    this.scene.activePie = obj;
    this.scene.activeSlice = null;
    this.currentSlice = null;
    this.currentPie = obj;
    if (this.currentPie) {
      this.scene.activePie.active = true;
  ...
  Piechart_Selection.prototype.onSceneChange = function(event) {
    ...
    if (event.changes.pie) {
      s1 = this.updateSelectionIds(this.scene.selection);
      if (s1 != null) {
        return this.setSelection(s1, this.scene.selectionIds, true, "data");
      }
    }
  };

  Piechart_Selection.prototype.updateSelectionIds = function(curSelection) {
    var changes, id, pie, selection, slice, slicesToSelect, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2;
    slicesToSelect = [];
    ...
    if (slicesToSelect.length > 0) {
      selection = curSelection.slice(0);
      for (_l = 0, _len3 = slicesToSelect.length; _l < _len3; _l++) {
        slice = slicesToSelect[_l];
        if (!Base_Helpers.arrayContains(selection, slice)) {
          changes = true;
  ...
  Piechart_Selection.prototype.setSelection = function(selection, selectionIds, alreadyChanged, origin) {
    var hasChanges, s, s1, _i, _j, _len, _len1, _ref, _ref1;
    ...
    hasChanges = alreadyChanged || this.scene.selection.length !== selection.length;
    if (selection.length === 1 && !hasChanges) {
      hasChanges = this.scene.selection[0] !== selection[0];
    }
    s1 = this.updateSelectionIds(selection);
    if (s1 != null) {
      selection = s1;
      hasChanges = true;
    }
    if (hasChanges) {
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        s = _ref1[_j];
        s.cutoutDistance = this.scene.settings.interaction.selection.cutoutDistance;
      }
      this.events.notifySceneChanges({
        selection: true
      });
...
Piechart_Data = (function() {
  ...
  Piechart_Data.prototype.canExapnd = function(id) {
    return this.pies.hasOwnProperty(id) || (Base_Helpers.isString(id) && id[0] !== "°");
  };

  Piechart_Data.prototype.getPieData = function(id, offset) {
    var r;
    if (this.pies.hasOwnProperty(id)) {
      r = this.filterAndSortData(this.pies[id]);
      if (r.length <= offset) {
        this.requestPieData(id, offset);
      }
      return r;
    } else if (id[0] === "°") {
      return null;
  ...
  Piechart_Data.prototype.requestMorePieData = function(id, offset) {
    if (offset == null) {
      offset = 0;
    }
  ...
  Piechart_Data.prototype.requestPieData = function(pieId, offset) {
    if (pieId[0] === "°") {
      throw "Invalid Id: " + pieId;
    }
    if (this.requestedPies.hasOwnProperty(pieId) || this.pendingPies[pieId] >= offset) {
      return;
    }
    this.pendingPies[pieId] = offset;
  ...
  Piechart_Data.prototype.runRequests = function() {
    var id, offset, r, remainingRequests, time, timeout, _i, _len, _ref, _ref1;
    if (this.removed) {
      return;
    }
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (r.time + timeout > time) {
          ...
          if (this.requestedPies.hasOwnProperty(id)) {
            offset = this.requestedPies[id];
            delete this.requestedPies[id];
            this.pendingPies[id] = offset;
          }
          if (r.time !== 0) {
    ...
    while (Base_Helpers.hasProperties(this.pendingPies) && this.requests.length < this.settings.data.numberOfParralelRequests) {
      _ref1 = Base_Helpers.removePropertyValue(this.pendingPies), id = _ref1[0], offset = _ref1[1];
      this.makeRequest(id, offset, time);
    }
  ...
  Piechart_Data.prototype.makeRequest = function(id, neededOffset, time) {
    var curData, failFunc, limit, offset, okFunc, request, response,
      _this = this;
    this.requestedPies[id] = neededOffset;
    this.chart.log("Requesting data on " + id);
    curData = this.pies[id];
    offset = curData ? curData.values.length : 0;
    limit = this.settings.data.itemsToLoad + Math.max(0, neededOffset - offset);
    request = {
      time: time,
      id: id,
      offset: offset,
    ...
    if ((response != null) && (Base_Helpers.isObject(response) || Base_Helpers.isString(response))) {
      return this.dataRecievedRaw(r, response);
    } else {
  ...
  Piechart_Data.prototype.dataArrived = function(request, rawData) {
    ...
    if (rawData) {
      ...
      if (data.error != null) {
        ...
        if (data.hasOwnProperty("offset") && parseInt(data.offset)) {
    ...
    if (allOk) {
      ...
      newIds = {};
      newIds[id] = true;
      this.addData(id, data.values, data.offset, data.limit, data.afterSum, data.beforeSum, data.sum, newIds);
      this.chart.events.notifySceneChanges({
        dataArrived: true,
  ...
  Piechart_Data.prototype.addData = function(id, values, offset, limit, afterValue, beforeValue, sum, newIds) {
    var pie, seenIds, v, _i, _len;
    seenIds = {};
    for (_i = 0, _len = values.length; _i < _len; _i++) {
      ...
      if (!v.id) {
        ...
        v.id = "" + v.id;
        if (seenIds.hasOwnProperty(v.id)) {
          this.chart.error("Data response: Multiple slices with the same ID: " + n.id);
        }
        seenIds[v.id] = true;
      }
      if (v.subvalues) {
        newIds[v.id] = true;
        this.addData(v.id, v.subvalues, 0, Infinity, 0, 0, 0, newIds);
      }
    }
    if (this.pies.hasOwnProperty(id)) {
      pie = this.pies[id];
    ...
    return pie.addData(values, offset, limit, afterValue, beforeValue, sum, this.chart);
  };
  ...
  Piechart_Data.prototype.filterAndSortData = function(pieData) {
    var cnt, filter, slice, total, values, _i, _len, _ref;
...
Piechart_Animations = (function() {
  ...
  Piechart_Animations.prototype.doAnimations = function(event) {
    var a0Animator, a1Animator, c, c0, c1, c2, c3, cc, changedPies, cutoutAnimator, dt, fadeDuration, fillAnimator, interactionDuration, lineAnimator, pie, pieChanges, piesToRemove, r0Animator, r1Animator, removePie, slice, slicesToRemove, time, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref10, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    if (this.prevTime) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (!removePie) {
        ...
        _ref1 = a0Animator.updateAndGet(pie.startAngle, time), pie.currentStartAngle = _ref1[0], c0 = _ref1[1];
        _ref2 = a1Animator.updateAndGet(pie.endAngle, time), pie.currentEndAngle = _ref2[0], c1 = _ref2[1];
        _ref3 = r1Animator.updateAndGet(pie.radius, time), pie.currentRadius = _ref3[0], c2 = _ref3[1];
        _ref4 = r0Animator.updateAndGet(pie.innerRadius, time), pie.currentInnerRadius = _ref4[0], c3 = _ref4[1];
        pie.currentRadius = Math.max(0.1, pie.currentRadius);
        pie.currentInnerRadius = Math.min(Math.max(0.1, pie.currentInnerRadius), pie.currentRadius);
        ...
        for (_j = 0, _len1 = _ref5.length; _j < _len1; _j++) {
          ...
          _ref6 = slice.animatorF.updateAndGetFixed(slice.fraction, time), slice.currentFraction = _ref6[0], c = _ref6[1];
          cc || (cc = c);
        ...
        for (_l = 0, _len3 = _ref7.length; _l < _len3; _l++) {
          ...
          _ref8 = lineAnimator.updateColorAndGet(slice.targetLineColor, time), slice.currentLineColor = _ref8[0], c0 = _ref8[1];
          _ref9 = fillAnimator.updateColorAndGet(slice.targetFillColor, time), slice.currentFillColor = _ref9[0], c1 = _ref9[1];
          if (!slice.userPlaced) {
            _ref10 = cutoutAnimator.updateAndGet(slice.cutoutDistance, time), slice.currentCutoutDistance = _ref10[0], c2 = _ref10[1];
          }
...
// Generated by CoffeeScript 1.6.3
var Piechart_LabelLayoutBase;

Piechart_LabelLayoutBase = (function() {
  function Piechart_LabelLayoutBase(g, config) {
    ...
    height = g.measureText("M").width * 1.2;
  ...
  Piechart_LabelLayoutBase.prototype.fitLabelInRect = function(label, align, locationFromHeight) {
  ...
  return Piechart_LabelLayoutBase;

})();

/*
//@ sourceMappingURL=LabelLayoutBase.map
*/
...
Linearchart_ValueAxis = (function() {
  ...
  Linearchart_ValueAxis.computeMargins = function(valueAxisList) {
    var axis, leftMargin, leftPadding, rightMargin, rightPadding, _i, _len;
    leftPadding = 0;
    rightPadding = 0;
    leftMargin = 0;
    rightMargin = 0;
    for (_i = 0, _len = valueAxisList.length; _i < _len; _i++) {
      ...
      if (axis.side === "left") {
        if (axis.position !== "inside") {
          ...
          leftPadding += axis.size;
        }
      } else {
        if (axis.position !== "inside") {
          rightMargin += axis.size;
        } else {
          rightPadding += axis.size;
        }
      }
    }
  ...
  function Linearchart_ValueAxis(r, name, options) {
    this.name = name;
  ...
  Linearchart_ValueAxis.prototype.computeNewScale = function(min, max) {
    var height, minStep, range, topPos, y0;
...
// Generated by CoffeeScript 1.6.3
var PieChart_Renderer_Flat, _ref,
...
PieChart_Renderer_Flat = (function(_super) {
  ...
  function PieChart_Renderer_Flat() {
    _ref = PieChart_Renderer_Flat.__super__.constructor.apply(this, arguments);
    return _ref;
  }
...
// Generated by CoffeeScript 1.6.3
var PieChart_Renderer_Can, _ref,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
...
PieChart_Renderer_Can = (function(_super) {
  ...
  function PieChart_Renderer_Can() {
    _ref = PieChart_Renderer_Can.__super__.constructor.apply(this, arguments);
    return _ref;
  }

  PieChart_Renderer_Can.prototype.paintPie = function(context, x, y, r0, r1, a0, a1, pie) {
    var depth, offset, _i, _len, _ref1, _results;
...
/*
//@ sourceMappingURL=Can.map
*/

// Generated by CoffeeScript 1.6.3
var PieChart_Renderer_Smoothy, _ref,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
...
PieChart_Renderer_Smoothy = (function(_super) {
  ...
  function PieChart_Renderer_Smoothy() {
    _ref = PieChart_Renderer_Smoothy.__super__.constructor.apply(this, arguments);
    return _ref;
  }
...
/*
//@ sourceMappingURL=Smoothy.map
*/

// Generated by CoffeeScript 1.6.3
var PieChart_Renderer_Bevel, _ref,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
...
PieChart_Renderer_Bevel = (function(_super) {
  ...
  function PieChart_Renderer_Bevel() {
    ...
    _ref = PieChart_Renderer_Bevel.__super__.constructor.apply(this, arguments);
    return _ref;
  }
...
Netchart_Scene = (function(_super) {
  ...
  */
  ...
  Netchart_Scene.prototype.activeNode = null;

  Netchart_Scene.prototype.activeLink = null;
  ...
  Netchart_Scene.prototype.removeNode = function(node) {
    var link, nodeId, _i, _len, _ref, _results;
    nodeId = node.id;
    if (!this.idToNode.hasOwnProperty(nodeId)) {
      return;
    }
  ...
  Netchart_Scene.prototype.getChartBounds = function() {
    return {
      left: this.x0,
      top: this.y0,
      right: this.x0 + this.width,
      bottom: this.y0 + this.height
    };
  };
  ...
  Netchart_Scene.prototype.getGraphBounds = function() {
    var node, r, x, x0, x1, y, y0, y1, _i, _len, _ref;
  ...
  Netchart_Scene.prototype.findLinkOrNodeAt = function(displayX, displayY, tolerance) {
    var best, cx, cy, distanceSq, insideSq, l, min, n, r, toleranceSq, x, x0, x1, y, y0, y1, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3;
    _ref = this.fromDisplay(displayX, displayY), x = _ref[0], y = _ref[1];
    min = tolerance;
    best = null;
    _ref1 = this.nodes;
    ...
    for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
      ...
      x1 = l.to.x;
      y1 = l.to.y;
      _ref3 = Base_Geometry.closestPointToLine(x0, y0, x1, y1, x, y), cx = _ref3[0], cy = _ref3[1];
...
Base_MouseEvents = (function() {
  ...
  */
  ...
  Base_MouseEvents.prototype.downHappened = function(e) {
    ...
    this.downPointers[e.identifier] = new Base_MousePointer(e, this.settings);
...
Linearchart_Settings = (function(_super) {
  ...
  function Linearchart_Settings(name) {
    ...
    this.apply(Linearchart_Settings.defaults);
  }

  Linearchart_Settings.prototype.getValueAxisList = function() {
...
// Generated by CoffeeScript 1.6.3
var Piechart_Icon, Piechart_Label, Piechart_Pie, Piechart_Scene, Piechart_Slice,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

Piechart_Label = (function() {
  function Piechart_Label() {}

  Piechart_Label.prototype.lines = null;

  Piechart_Label.prototype.words = null;

  Piechart_Label.prototype.lineHeight = 0;

  Piechart_Label.prototype.align = "left";

  Piechart_Label.prototype.hwidth = 0;

  Piechart_Label.prototype.hheight = 0;

  Piechart_Label.prototype.x = null;

  Piechart_Label.prototype.y = null;

  Piechart_Label.prototype.angle = 0;

  Piechart_Label.prototype.visible = true;

  Piechart_Label.prototype.userPlaced = false;

  Piechart_Label.prototype.inside = true;

  return Piechart_Label;
...
Piechart_Pie = (function() {
  ...
  Piechart_Pie.prototype.columnsInScreen = 10;
  ...
  Piechart_Pie.prototype.getActiveSlice = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      slice = _ref[_i];
      if (slice.data.id === this.activeSliceId) {
        return slice;
      }
    }
    return null;
  ...
  Piechart_Pie.prototype.radius = 100;

  Piechart_Pie.prototype.innerRadius = 0;
...
Piechart_Scene = (function(_super) {
  ...
  Piechart_Scene.prototype.getChartBounds = function() {
    return {
      left: 0,
      top: 0,
      right: this.x0 + this.width,
    ...
    if (parent && !pie.parentSlice) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (slice.data.id === pie.id) {
          ...
          parent.activeSliceId = slice.data.id;
          break;
        }
      }
    }
    if (child && !child.parentSlice) {
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        ...
        if (slice.id === child.id) {
          ...
          pie.activeSliceId = slice.data.id;
          break;
        } else {
          _results.push(void 0);
        }
...
Piechart_Styles = (function() {
  ...
  Piechart_Styles.prototype.updateSlice = function(slice, index) {
    var brightness, pie, targetFillColor, targetLineColor;
    pie = slice.pie;
    if (slice.removed) {
      return;
    }
    ...
    if (Base_Helpers.arrayContains(this.scene.selection, slice)) {
      Base_Helpers.extend(slice, this.scene.settings.slice.selectedStyle);
    }
    if (this.scene.activeSlice === slice) {
      Base_Helpers.extend(slice, this.scene.settings.slice.hoverStyle);
    }
    if (slice.data.style) {
      Base_Helpers.extend(slice, slice.data.style);
    }
    if (this.scene.settings.slice.styleFunction) {
      this.scene.settings.slice.styleFunction(slice, slice.data);
    } else {
...
Piechart_Layers = (function() {
  ...
  function Piechart_Layers(chart) {
    ...
    this.canvas = Base_Helpers.createDom("canvas", "Piechart-canvas", null, this.container);
...
Base_ChartEvents = (function() {
  ...
  Base_ChartEvents.prototype.rcl = function(event) {
    this._notifyMouseEvent(event, "RightClick");
    return this.notifySceneChanges(event.changes);
  };
  ...
  Base_ChartEvents.prototype.exportGetDimensions = function(dimensions) {
    ...
    if (width > 0 && height > 0) {
      ...
      scale = dimensions.scaling > 0 ? dimensions.scaling : 1;
    ...
    if (dimensions.unit === "mm") {
      if (dimensions.dpi > 0 && dimensions.width > 0 && dimensions.height > 0) {
        pixelsPerMM = dimensions.dpi / 25.4;
        scale *= pixelsPerMM;
        width *= pixelsPerMM;
        height *= pixelsPerMM;
      } else {
  ...
  Base_ChartEvents.prototype.exportToImage = function(type, dimensions, transparent) {
    var bounds, canvas, context, e, event, height, mime, mime_map, scale, scaleX, scaleY, width, _i, _len, _ref, _ref1;
    mime_map = {
      "png": "image/png",
      "jpeg": "image/jpeg"
    };
    mime = mime_map[type];
    _ref = this.exportGetDimensions(dimensions), width = _ref[0], height = _ref[1], scale = _ref[2];
...
Base_Credits = (function() {
  ...
  Base_Credits.prototype.paintScene = function(event) {
    ...
    if (i && (i.width || event["export"])) {
      ...
      w = canvas.width;
      h = canvas.height;
      context.drawImage(i, w - i.width - margin, h - i.height - margin);
...
Base_InfoPopup = (function() {
  ...
  Base_InfoPopup.prototype.show = function(x, y, contents) {
    if (!this.popupVisible) {
    ...
    return this.updateXY(x, y);
  };

  Base_InfoPopup.prototype.updateContents = function(contents) {
    this.popup.innerHTML = contents;
    return this.updateXY(this.x, this.y);
  };

  Base_InfoPopup.prototype.updateXY = function(x, y) {
    var arrowWidth, cl, h, margin, w;
    this.x = x;
    this.y = y;
    margin = 12;
    arrowWidth = 10;
    ...
    if (x + w > this.scene.x0 + this.scene.width + arrowWidth) {
      x = x - w - arrowWidth;
      cl = "DVSL-info-left";
    } else {
      x = x + arrowWidth;
...
Piechart_PieLayout = (function() {
  ...
  Piechart_PieLayout.prototype.placeStack = function(pie) {
    var depth, endAngle, height, i, innerRadius, pieChanges, pieIndex, pies, r, radius, radiusStep, remainingSlices, settings, startAngle, x, y, _i, _j, _len, _ref, _ref1, _ref2;
    ...
    if (!radius) {
      r = Math.min(height / 2, this.scene.width / 2);
      r -= settings.pie.outerMargin;
      if (settings.labels.enabled) {
        r -= settings.labels.connectorLength * 0.5;
      }
      radius = r;
    }
    ...
    if (pie.x !== x || pie.y !== y || pie.radius !== radius || pie.innerRadius !== innerRadius || startAngle !== pie.startAngle || endAngle !== pie.endAngle) {
      pieChanges = true;
      pie.x = x;
      pie.y = y;
      pie.radius = radius;
    ...
    if (pieIndex > 0) {
      ...
      radiusStep = Math.max(0, Math.min(radius - innerRadius, (innerRadius - settings.pie.innerMargin) / remainingSlices));
      for (i = _i = _ref = pieIndex - 1; _i >= 0; i = _i += -1) {
        ...
        pie.y = y;
        pie.radius = radius;
        pie.innerRadius = innerRadius + settings.pie.margin;
  ...
  Piechart_PieLayout.prototype.placeLabels = function(pie, g) {
    var a0, a1, am, astart, bottom, config, da, endAngle, fsum, icon, iconMinSize, iconSize, insideLablelProp, label, labelLayout, left, leftOutsideLabels, margin, r0, r1, right, rightOutsideLabels, slice, sliceAngle0, sliceAngle1, startAngle, text, top, x, y, _i, _len, _ref, _ref1;
    ...
    x = pie.x;
    y = pie.y;
    r0 = pie.currentInnerRadius;
    r1 = pie.currentRadius;
    fsum = 0;
    fsum = 0;
    ...
    labelLayout = new Piechart_LabelLayout(g, r1, left, right, top, bottom, config);
    leftOutsideLabels = [];
    rightOutsideLabels = [];
    sliceAngle1 = astart;
    _ref = pie.allSlices;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (!slice.removed && !pie.background) {
        if (slice.insideLabel) {
          text = slice.insideLabel;
          if (!slice.currentInsideLabel) {
            label = slice.currentInsideLabel = new Piechart_Label();
          } else {
          ...
          if (!label.shouldBeVisible && slice !== this.scene.activeSlice && !label.userPlaced) {
            label.visible = false;
          }
        }
        if (slice.label) {
          text = slice.label;
          if (config.insideLabel === "append" && slice.insideLabel && !slice.currentInsideLabel.userPlaced && !slice.currentInsideLabel.shouldBeVisible) {
            x = Math.cos(am);
            if (x > 0) {
          ...
          if (!slice.currentLabel) {
            label = slice.currentLabel = new Piechart_Label();
          } else {
    ...
    labelLayout.computeOutsideLabelLocations(rightOutsideLabels, 0);
    return labelLayout.computeOutsideLabelLocations(leftOutsideLabels, 1);
  };
...
Piechart_LabelLayout = (function(_super) {
  ...
  function Piechart_LabelLayout(g, r1, left, right, top, bottom, config) {
    this.left = left;
    this.right = right;
    this.top = top;
    this.bottom = bottom;
    Piechart_LabelLayout.__super__.constructor.call(this, g, config);
  ...
  Piechart_LabelLayout.prototype.computeOutsideLabelLocations = function(labels, direction) {
    var aligned, bottom, h, hasMultiline, height, label, labelBottom, labelSpacing, labelTop, maxHeight, prop, sortDirection, top, visibleLabels, worstProp, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _results;
    if (direction == null) {
      direction = 1;
    }
    ...
    sortDirection = 0.5 - direction;
    labels.sort(function(a, b) {
      return (a.y - b.y) * sortDirection;
    });
    ...
    for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
      ...
      for (_m = 0, _len4 = visibleLabels.length; _m < _len4; _m++) {
        label = visibleLabels[_m];
        if (direction === 1) {
  ...
  Piechart_LabelLayout.prototype.fitLabelInSlice = function(label, r0, r1, a0, a1) {
    var sliceLocationFunc,
      _this = this;
    sliceLocationFunc = function(hheight) {
  ...
  Piechart_LabelLayout.prototype.placeOutsideMaxWidth = function(x, y, hheight, aligned) {
    ...
    distanceFromCenter += this.connectorLength;
    if (x > 0) {
  ...
  Piechart_LabelLayout.prototype.placeInsideMaxWidth = function(hheight, r0, r1, a0, a1) {
  ...
  Piechart_LabelLayout.prototype.getInsideMaxWidth = function(x, y, hheight, r0, r1, a0, a1, userPlaced) {
...
Piechart_PieRenderer = (function() {
  ...
  Piechart_PieRenderer.prototype.doAnimations = function(event) {};
  ...
  Piechart_PieRenderer.prototype.paintScene = function(event) {
    ...
    if (Base_Helpers.arrayContains(this.scene.pies, this.scene.activePie)) {
  ...
  Piechart_PieRenderer.prototype.paintPie = function(g, pie) {
    var a0, a1, aR1, am, angle, angle1, angleDiff, end, h, icon, img, innerAngleLeft, innerAngleRight, m, outerAngleLeft, outerAngleRight, pxToAngleR0, pxToAngleR1, r0, r1, rl, slice, sliceAngle, slicesToPaint, spacing, spacingAngleR0, spacingAngleR1, sr0, sr1, start, w, x, xx, y, yy, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _results;
    spacing = this.settings.slice.margin;
    x = pie.x;
    y = pie.y;
    r0 = Math.max(pie.currentInnerRadius, spacing / 2);
    r1 = pie.currentRadius;
    ...
    if (!pie.removed) {
      ...
      for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
        slice = _ref1[_k];
        if (slice.currentIcon && (img = this.scene.settings.getAssetImage(slice.icon))) {
      ...
      for (_l = 0, _len3 = _ref2.length; _l < _len3; _l++) {
        ...
        if (slice.currentInsideLabel && slice.currentInsideLabel.visible) {
          this.paintLabel(g, x, y, rl, am, slice.currentInsideLabel, this.settings.labels.insideTextStyle);
        }
        if (slice.currentLabel && slice.currentLabel.visible) {
          _results.push(this.paintLabel(g, x, y, rl, am, slice.currentLabel, this.settings.labels.textStyle));
        } else {
          _results.push(void 0);
        }
      }
  ...
  Piechart_PieRenderer.prototype.paintLabel = function(g, x, y, r1, angleM, label, style) {
    var a, bottom, cosa, da, hh, labelConfig, line, r15, r2, sina, t, top, touchAngle, ww, x0, x1, x1a, x2, xc, xx, y0, y1, y1a, y2, yc, yy, _i, _len, _ref;
    top = this.scene.y0;
    bottom = top + this.scene.height;
...
Piechart_PieBuilder = (function() {
  ...
  Piechart_PieBuilder.prototype.updatePie = function(pie) {
    var data, newSlices, numberOfSlices, others, previous, _ref;
    data = this.getPieData(pie);
    numberOfSlices = this.getForwardCount(data, pie.offset);
    if (this.loading) {
      this.scene.data.requestMorePieData(pie.id, pie.offset);
    }
    pie.data = data;
    _ref = this.buildSlices(pie, pie.offset, numberOfSlices), newSlices = _ref[0], others = _ref[1], previous = _ref[2];
    if (newSlices.length > 0 || !this.loading) {
  ...
  Piechart_PieBuilder.prototype.scrollForward = function(pie, newOffset) {
    var data, newOthers, newPrevious, newSlices, numberOfSlices, _ref;
    data = this.getPieData(pie);
    if (newOffset == null) {
      numberOfSlices = this.getForwardCount(data, pie.offset);
      newOffset = pie.offset + numberOfSlices;
    }
    numberOfSlices = this.getForwardCount(data, newOffset);
    if (this.loading) {
      this.scene.data.requestMorePieData(pie.id, pie.offset);
    }
    pie.data = data;
    _ref = this.buildSlices(pie, newOffset, numberOfSlices), newSlices = _ref[0], newOthers = _ref[1], newPrevious = _ref[2];
    if (pie.othersSlice) {
  ...
  Piechart_PieBuilder.prototype.scrollBackward = function(pie, newOffset) {
    ...
    data = this.getPieData(pie);
    if (newOffset == null) {
      newOffset = this.getBackOffset(data, pie.offset);
    }
    numberOfSlices = this.getForwardCount(data, newOffset);
    if (this.loading) {
      this.scene.data.requestMorePieData(pie.id, pie.offset);
  ...
  Piechart_PieBuilder.prototype.getForwardCount = function(data, offset) {
    var count, i, maxOthers, maxSlices, minSlice, othersRealFraction, othersS, othersSum, othersVisibleFraction, sliceFraction, slicesSum, val, _i, _ref;
    if (!data || (data.values.length <= offset && (!data.done))) {
      this.loading = true;
      return 0;
    }
    ...
    if (!othersS.enabled) {
      ...
      return Math.max(data.values.length - offset, 0);
    }
    othersSum = data.afterFraction;
    for (i = _i = offset, _ref = data.values.length - 1; _i <= _ref; i = _i += 1) {
      othersSum += data.values[i].fraction;
    }
    ...
    count = 0;
    slicesSum = 0;
    while (count + offset < data.values.length) {
      count++;
      val = data.values[offset + count - 1].fraction;
      slicesSum += val;
      othersSum -= val;
      ...
      if (count > maxSlices || sliceFraction < minSlice) {
        if (count + offset !== data.values.length) {
          count--;
        }
        break;
      }
    }
    this.loading = !data.done && offset + maxSlices > data.values.length;
    return count;
  };

  Piechart_PieBuilder.prototype.getBackOffset = function(data, othersOffset) {
    var count, i, maxOthers, maxSlices, minSlice, othersFraction, othersS, othersSum, pos, slicesSum, smallestSliceFraction, smallestValue, val, _i, _ref;
    othersS = this.scene.settings.interaction.others;
    if (!data) {
      this.loading = true;
      return 0;
    ...
    othersSum = data.afterFraction;
    for (i = _i = othersOffset, _ref = data.values.length - 1; _i <= _ref; i = _i += 1) {
      othersSum += data.values[i].fraction;
    }
    minSlice = othersS.minSliceFraction;
    maxOthers = othersS.maxOthersFraction;
    maxSlices = othersS.maxSlicesVisible;
    pos = othersOffset;
    smallestValue = data.values[othersOffset - 1].fraction;
    slicesSum = 0;
    while (pos > 0) {
      pos--;
      count = othersOffset - pos;
      val = data.values[pos].fraction;
      slicesSum += val;
      ...
      if (count > maxSlices || smallestSliceFraction < minSlice) {
        if (pos > 0) {
          pos++;
        }
        break;
  ...
  Piechart_PieBuilder.prototype.buildSlices = function(pie, offset, count) {
    var data, fractionCompress, i, othersFraction, othersSettings, othersSlice, othersVal, prevValue, previousFraction, previousSlice, s, slice, slices, slicesFraction, _i, _j, _k, _l, _len, _ref, _ref1, _ref2, _ref3;
    data = pie.data;
    if (!data || (data.values.length < offset && !data.ready)) {
      this.loading = true;
      return [[], null, null];
    }
    slicesFraction = 0;
    slices = [];
    for (i = _i = offset, _ref = Math.min(offset + count, data.values.length) - 1; _i <= _ref; i = _i += 1) {
      s = new Piechart_Slice(pie, i);
      s.data = data.values[i];
      s.id = s.data.id;
    ...
    for (i = _j = _ref1 = offset + count, _ref2 = data.values.length - 1; _j <= _ref2; i = _j += 1) {
      othersVal += data.values[i].value;
      othersFraction += data.values[i].fraction;
    }
    if (othersFraction > 0) {
    ...
    if (offset > 0) {
      prevValue = 0;
      for (i = _k = 0, _ref3 = Math.min(offset, data.values.length) - 1; _k <= _ref3; i = _k += 1) {
        prevValue += Math.abs(data.values[i].value);
      }
  ...
  Piechart_PieBuilder.prototype.getPieData = function(pie) {
    ...
    data = this.scene.data.getPieData(pie.id, pie.offset);
    if (!data) {
      return null;
    }
    sum = data.total;
...
// Generated by CoffeeScript 1.6.3
var Piechart_ColumnLabelLayout, Piechart_ColumnLayout,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

Piechart_ColumnLayout = (function() {
  "use strict";
  function Piechart_ColumnLayout(chart, view) {
    this.chart = chart;
    this.view = view;
    this.scene = chart.scene;
    this.events = chart.events;
  ...
  Piechart_ColumnLayout.prototype.placeColumns = function(pie) {
    var activeSlice, i, index, maxSlicesInScreen, pieChanges, pieIndex, pies, sliceWidth, slices, slicesInScreen, w, _i, _j, _len, _ref, _ref1, _ref2;
    pieChanges = false;
    pie.startAngle = 0;
    pie.endAngle = 1;
    slices = pie.count;
    w = this.scene.width;
    if (slices > 0) {
      sliceWidth = Math.min(this.scene.settings.columns.maxColumnWidth, Math.max(this.scene.settings.columns.minColumnWidth, w / slices));
    } else {
      sliceWidth = this.scene.settings.columns.maxColumnWidth;
    }
    maxSlicesInScreen = Math.round(this.scene.width / sliceWidth);
    slicesInScreen = Math.max(1, Math.min(maxSlicesInScreen, pie.allSlices.length));
    if (slicesInScreen !== pie.columnsInScreen) {
      pie.columnsInScreen = slicesInScreen;
      pieChanges = true;
    }
    pies = this.scene.stack;
    pieIndex = pies.length - 1;
    if (pieIndex > 0) {
      for (i = _i = 0, _ref = pieIndex - 1; _i <= _ref; i = _i += 1) {
        pie = pies[i];
        activeSlice = pie.getActiveSlice();
        if (activeSlice) {
          index = pie.allSlices.indexOf(activeSlice);
          pie.startAngle = -index;
          pie.endAngle = pie.columnsInScreen - index;
          pie.scrollOffset = 0;
        } else {
          pie.startAngle = -1;
          pie.endAngle = 0;
  ...
  Piechart_ColumnLayout.prototype.placeLabels = function(pie, valueAxis, g) {
    var bottom, config, fsum, iconMinSize, index, insideLablelProp, label, labelLayout, left, margin, right, slice, text, top, x, y, _i, _len, _ref, _results;
    config = this.scene.settings.labels;
    if (!config.enabled) {
      return;
    }
    ...
    top = this.scene.y0 - y;
    bottom = top + this.scene.height;
    labelLayout = new Piechart_ColumnLabelLayout(g, this.view.getColumnTransform(pie), valueAxis, left, right, top, bottom, bottom + this.scene.settings.columns.labelsHeight, config);
    _ref = pie.allSlices;
    _results = [];
    for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
      ...
      if (!slice.removed && !pie.background) {
        if (slice.insideLabel) {
          ...
          if (!label.userPlaced) {
            insideLablelProp = labelLayout.placeInsideLabel(label, slice.value, index);
            if (config.insideLabel === "auto" && insideLablelProp < 1 && slice !== this.scene.activeSlice) {
              label.visible = false;
            }
          }
        }
        if (slice.label) {
          text = slice.label;
          if (slice.insideLabel && !slice.currentInsideLabel.visible) {
            text += ", " + slice.insideLabel;
          }
          label = null;
          if (!slice.currentLabel) {
          ...
          if (!label.userPlaced) {
            _results.push(labelLayout.placeOutsideLabel(label, slice.value, index));
          } else {
            _results.push(void 0);
          }
        } else {
          _results.push(slice.currentLabel = null);
        }
      } else {
        slice.currentLabel = null;
        _results.push(slice.currentInsideLabel = null);
      }
    }
    return _results;
  };

  return Piechart_ColumnLayout;

})();

Piechart_ColumnLabelLayout = (function(_super) {
  __extends(Piechart_ColumnLabelLayout, _super);

  function Piechart_ColumnLabelLayout(g, transform, valueAxis, left, right, top, bottom, bottomLabels, config) {
    ...
    Piechart_ColumnLabelLayout.__super__.constructor.call(this, g, config);
    this.trAdd = transform[0], this.trMul = transform[1];
  }

  Piechart_ColumnLabelLayout.prototype.placeInsideLabel = function(label, value, index) {
    var p1, x0, x1, y0, y1;
    x0 = 0;
    x1 = this.trMul;
    y0 = this.valueAxis.zeroY - this.bottom;
    y1 = y0 - this.valueAxis.valueToRelativeY(value);
    p1 = this.placeLabel(label, x0, Math.max(this.top - this.bottom, Math.min(y0, y1)), x1, Math.min(0, Math.max(y0, y1)), "top");
    if (p1 >= 1) {
      return p1;
    }
    return this.placeLabel(label, x0, this.top - this.bottom, x1, Math.max(this.top - this.bottom, Math.min(y0, y1)), "bottom");
  };

  Piechart_ColumnLabelLayout.prototype.placeLabel = function(label, left, top, right, bottom, valign) {
    var locationFromHeight;
    locationFromHeight = function(halfHeight) {
      ...
      if (halfHeight * 2 <= bottom - top) {
        width = right - left;
      } else {
        width = 0;
      }
      return [centerX, centerY, 1, 0, width];
    };
    return this.fitLabelInRect(label, "center", locationFromHeight);
  };

  Piechart_ColumnLabelLayout.prototype.placeOutsideLabel = function(label, value, index) {
    var align, angle, availableHeight, availableWidth, bottom, cosa, left, locationFromHeight, middleX, right, sina, top;
    left = 0;
    right = this.trMul;
    top = 0;
    bottom = this.bottomLabels - this.bottom;
    angle = label.angle = this.angle;
    if (angle > 0) {
    ...
    locationFromHeight = function(halfHeight) {
      ...
      if (angle > 0) {
        ...
        centerY = top + halfHeight * cosa + width / 2 * sina;
      } else if (angle < 0) {
        centerX = middleX - width / 2 * cosa;
        centerY = top + halfHeight * cosa - width / 2 * sina;
      } else {
    ...
    return this.fitLabelInRect(label, align, locationFromHeight);
  };

  return Piechart_ColumnLabelLayout;
...
// Generated by CoffeeScript 1.6.3
var Piechart_ColumnRenderer;

Piechart_ColumnRenderer = (function() {
  "use strict";
  function Piechart_ColumnRenderer(chart, view) {
    var k, v, _ref;
    this.chart = chart;
    this.view = view;
    this.settings = chart.settings;
    ...
    for (k in _ref) {
      v = _ref[k];
      this.valueAxis[k] = new Linearchart_ValueAxis(this, k, v);
    }
  }

  Piechart_ColumnRenderer.prototype.doAnimations = function(event) {
    var f0, f1, max, min, name, pie, sliceFraction, v, va, x, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _results;
    if (event.changes.pie || event.changes.settings || event.changes.bounds) {
      va = this.valueAxis["default"];
      va.series = 0;
      _ref = this.scene.pies;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        pie = _ref[_i];
        ...
        max = -Infinity;
        f0 = pie.scrollOffset;
        sliceFraction = 1 / pie.columnsInScreen;
        _ref1 = pie.slices;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          v = _ref1[_j];
          f1 = f0 + sliceFraction;
        ...
        va.recieveSeriesGeometry(min, max, 0);
      }
    }
  ...
  Piechart_ColumnRenderer.prototype.paintScene = function(event) {
    var g, h, name, o, pie, pieindex, w, x, y, _i, _j, _len, _len1, _ref, _ref1, _ref2;
    g = event.context;
    x = this.scene.x0;
    w = this.scene.width;
    y = this.scene.y0;
    h = this.scene.height + this.scene.settings.columns.labelsHeight;
    _ref = this.valueAxis;
    for (name in _ref) {
      o = _ref[name];
      o.paint(g, g);
    }
    Base_Graphics.pushClip(g, x, y, w, h);
    _ref1 = this.scene.pies;
    for (pieindex = _i = 0, _len = _ref1.length; _i < _len; pieindex = ++_i) {
      pie = _ref1[pieindex];
      this.paintColumns(g, pie);
    }
    _ref2 = this.scene.pies;
    for (pieindex = _j = 0, _len1 = _ref2.length; _j < _len1; pieindex = ++_j) {
      pie = _ref2[pieindex];
      if (!(pie.background || pie.removed)) {
        this.paintLabels(g, pie);
      }
    }
    return Base_Graphics.popClip(g);
  };

  Piechart_ColumnRenderer.prototype.paintColumns = function(g, pie) {
    var fc, index, lc, left, margin, right, sleft, slice, sright, trAdd, trMul, va, vy, ya, yb, zeroY, _i, _len, _ref, _ref1, _results;
    va = this.valueAxis["default"];
    zeroY = va.zeroY;
    _ref = this.view.getColumnTransform(pie), trAdd = _ref[0], trMul = _ref[1];
    margin = this.scene.settings.slice.margin;
    margin = Math.min(trMul / 2 - 1, Math.max((trMul - this.scene.settings.columns.maxColumnWidth) / 2, margin));
    sleft = this.scene.x0;
    sright = sleft + this.scene.width;
    left = trAdd + margin;
    right = Math.max(left + 1, trAdd + trMul - margin);
    _ref1 = pie.allSlices;
    _results = [];
    for (index = _i = 0, _len = _ref1.length; _i < _len; index = ++_i) {
      slice = _ref1[index];
      if (left < sright && right > sleft) {
        ya = zeroY - slice.currentCutoutDistance;
        vy = va.valueToRelativeY(slice.value);
        if (vy < 0) {
          yb = ya - vy;
        } else {
          yb = ya;
      ...
      left += trMul;
      _results.push(right += trMul);
    }
    return _results;
  };

  Piechart_ColumnRenderer.prototype.paintLabels = function(g, pie) {
    var index, slice, trAdd, trMul, x, y, _i, _len, _ref, _ref1, _results;
    y = this.scene.y0 + this.scene.height;
    _ref = this.view.getColumnTransform(pie), trAdd = _ref[0], trMul = _ref[1];
    _ref1 = pie.allSlices;
    _results = [];
    for (index = _i = 0, _len = _ref1.length; _i < _len; index = ++_i) {
      ...
      x = trAdd + index * trMul;
      if (slice.currentInsideLabel && slice.currentInsideLabel.visible) {
        this.paintLabel(g, x, y, slice.currentInsideLabel, this.settings.labels.insideTextStyle);
      }
      if (slice.currentLabel && slice.currentLabel.visible) {
        _results.push(this.paintLabel(g, x, y, slice.currentLabel, this.settings.labels.textStyle));
      } else {
        _results.push(void 0);
      }
    }
  ...
  Piechart_ColumnRenderer.prototype.paintLabel = function(g, x, y, label, style) {
    var line, x0, y0, _i, _len, _ref;
    g.save();
    g.translate(x + label.x, y + label.y);
    g.rotate(label.angle);
    x0 = 0;
    y0 = -label.hheight;
    g.textBaseline = "top";
    if (label.align === "left") {
  ...
  return Piechart_ColumnRenderer;
...
// Generated by CoffeeScript 1.6.3
var Piechart_ColumnBuilder;

Piechart_ColumnBuilder = (function() {
  "use strict";
  function Piechart_ColumnBuilder(chart) {
    this.chart = chart;
    this.scene = chart.scene;
    this.loading = false;
  }

  Piechart_ColumnBuilder.prototype.updatePie = function(pie) {
    var data, newSlices, numberOfSlices, others, previous, _ref;
    data = this.getPieData(pie);
    numberOfSlices = this.getForwardCount(data, pie.offset);
    if (this.loading) {
  ...
  Piechart_ColumnBuilder.prototype.scrollForward = function(pie, newOffset) {
    var data, newOthers, newPrevious, newSlices, numberOfSlices, _ref;
    data = this.getPieData(pie);
    if (newOffset == null) {
      numberOfSlices = this.getForwardCount(data, pie.offset);
  ...
  Piechart_ColumnBuilder.prototype.scrollBackward = function(pie, newOffset) {
    var data, newOthers, newPrevious, newSlices, numberOfSlices, _ref;
    data = this.getPieData(pie);
    if (newOffset == null) {
      newOffset = this.getBackOffset(data, pie.offset);
    }
    numberOfSlices = pie.count + newOffset - pie.offset;
    if (!data || (!data.done && pie.offset + numberOfSlices > data.values.length)) {
      this.scene.data.requestMorePieData(pie.id, pie.offset);
      this.loading = true;
    } else {
      this.loading = false;
  ...
  Piechart_ColumnBuilder.prototype.getForwardCount = function(data, offset) {
    var count, slicesToDisplay;
    if (!data || (data.values.length <= offset && (!data.done))) {
      this.loading = true;
      return 0;
    }
    slicesToDisplay = Math.ceil(this.scene.width / this.scene.settings.columns.minColumnWidth);
    count = Math.max(0, Math.min(slicesToDisplay, data.values.length - offset));
    this.loading = !data.done && offset + slicesToDisplay > data.values.length;
    return count;
  };

  Piechart_ColumnBuilder.prototype.getBackOffset = function(data, othersOffset) {
    var offset, slicesToDisplay;
    if (!data) {
      this.loading = true;
      return 0;
    }
    slicesToDisplay = Math.ceil(this.scene.width / this.scene.settings.columns.minColumnWidth);
    offset = Math.max(0, othersOffset - slicesToDisplay);
    if (offset === 1) {
      offset = 0;
    }
    return offset;
  ...
  Piechart_ColumnBuilder.prototype.buildSlices = function(pie, offset, count) {
    var data, fractionCompress, i, othersFraction, othersSettings, othersSlice, othersVal, prevValue, previousFraction, previousSlice, s, slice, slices, slicesFraction, _i, _j, _k, _l, _len, _ref, _ref1, _ref2, _ref3;
    data = pie.data;
    if (!data || (data.values.length < offset && !data.ready)) {
      this.loading = true;
  ...
  Piechart_ColumnBuilder.prototype.applyNewSlices = function(pie, newSlices, others, previous) {
    var allSlices, i1, i2, oldSlices, s1, s2, slice, slices, wasEmpty, _i, _len, _ref;
    oldSlices = pie.slices;
    wasEmpty = oldSlices.length === 0;
    slices = [];
  ...
  Piechart_ColumnBuilder.prototype.getPieData = function(pie) {
    var data;
    data = this.scene.data.getPieData(pie.id, pie.offset);
    if (!data) {
      return null;
...
Piechart_Settings = (function(_super) {
  ...
  Piechart_Settings.defaults = {
    ...
    advanced: {
      renderQuality: 1,
      backImage: "builtin-back",
      assets: ["timechart.css", "piechart.css", "jspdf/jspdf.min.js", "jspdf/jspdf.plugin.addimage.min.js"],
      iconMinSize: 16,
      animationTime: 500,
      interactionTime: 200,
      ...
      builtinAssets: {
        "builtin-back": Piechart_images_back
      }
    },
    events: {
      onClick: null,
  ...
  function Piechart_Settings(settings) {
    ...
    this.apply(Piechart_Settings.defaults);
    this.apply(settings);
  }
...
Base_Impl = (function() {
  ...
  Base_Impl.prototype.updateSize = function(force) {
    ...
    if (!clientWidth) {
      clientWidth = Math.max(this.settings.minWidth, Math.min(this.settings.maxWidth, this.layers.container.clientWidth));
    }
    if (!clientHeight) {
      clientHeight = Math.max(this.settings.minHeight, Math.min(this.settings.maxHeight, this.layers.container.clientHeight));
    }
  ...
  Base_Impl.prototype.dispatchEvent = function(name, event, defaultFunc, delay) {
    ...
    if (this.eventListeners[name] != null) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        l = _ref[_i];
        this.log("Call user event handler, " + name);
...
Base_Title = (function() {
  ...
  Base_Title.prototype.paintScene = function(event) {
    var enabled, g, h, left, line, margin, right, title, top, w, x, y, _i, _len, _ref, _ref1, _results;
    title = this.scene.settings.title;
...
Piechart_Navigator = (function() {
  ...
  Piechart_Navigator.prototype.setPie = function(id, offset, origin) {
    ...
    if (pie.offset !== offset) {
      loading = this.scroll(pie, offset);
      changes++;
    } else if (changes && pie) {
      loading = this.scene.view.updatePie(pie);
    }
    if (changes) {
      this.chart.notifyPieChange(pie, origin);
      if (!loading) {
        this.chart.notifyPieReady(pie, origin);
      }
  ...
  Piechart_Navigator.prototype.goToPie = function(pie, origin) {
    var changes;
    if (!pie) {
      return;
    }
    changes = 0;
    while (this.scene.peek() !== pie) {
      this.scene.pop();
      changes++;
    }
    if (changes) {
  ...
  Piechart_Navigator.prototype.goOthers = function(pie, origin) {
    ...
    loading = this.scene.view.scrollForward(pie, pie.offset + pie.count);
    this.chart.notifyPieChange(pie, origin);
    if (!loading) {
      this.chart.notifyPieReady(pie, origin);
    }
  ...
  Piechart_Navigator.prototype.goPrevious = function(pie, origin) {
    ...
    loading = this.scene.view.scrollBackward(pie);
    this.chart.notifyPieChange(pie, origin);
    if (!loading) {
      this.chart.notifyPieReady(pie, origin);
    }
  ...
  Piechart_Navigator.prototype.scroll = function(pie, offset) {
    var loading;
    if (pie.offset > offset) {
      loading = this.scene.view.scrollBackward(pie, offset);
    } else if (pie.offset < offset) {
      loading = this.scene.view.scrollForward(pie, offset);
    } else {
      loading = false;
    }
    return loading;
  ...
  Piechart_Navigator.prototype.onSceneChange = function(event) {
    var activePie, activePieReady, ids, loading, pie, _i, _j, _len, _len1, _ref, _ref1;
    ...
    if (event.changes.dataArrived) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (ids.hasOwnProperty(pie.id)) {
          ...
          activePieReady || (activePieReady = pie === activePie && !loading);
        }
      }
    }
    if (event.changes.data) {
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        ...
        if (!pie.data || pie === activePie || event.changes.dataDeep) {
          ...
          event.changes.pie = true;
          if (pie === activePie) {
            activePieReady = !loading;
          }
        }
      }
    }
    if (activePieReady) {
      return this.chart.notifyPieReady(activePie, "data");
    }
  };
  ...
  Piechart_InfoPopup.prototype.setCurrentObject = function(x, y, obj) {
    ...
    if (!obj) {
      ...
      if (obj !== this.currentSlice) {
        ...
        this.popup.show(x, y, contents);
      } else {
        this.popup.updateXY(x, y);
      }
...
Piechart_PieView = (function(_super) {
  ...
  Piechart_PieView.prototype.doAnimations = function(event) {
    ...
    if (event.changes.settings || event.changes.pie || event.changes.bounds) {
      ...
      pie = this.scene.peek();
      if (pie != null) {
        pieChanges || (pieChanges = this.layout.placeStack(pie));
      }
    }
    ...
    return this.renderer.doAnimations(event);
  };
  ...
  Piechart_PieView.prototype.findPieAt = function(px, py, tolerance) {
    var distSq, dx, dy, i, pie, r0, r0sq, r1, r1sq, x, xx, y, _i, _ref;
  ...
  Piechart_PieView.prototype.findSliceAt = function(px, py, tolerance) {
    var a1, ad, cosa0, cosa1, dist, distSq, dp1, dp2, dx, dy, i, pie, r0, r0sq, r1, r1sq, selectionDistance, sina0, sina1, slice, x, xx, y, _i, _j, _len, _ref, _ref1;
...
/*
//@ sourceMappingURL=PieView.map
*/

// Generated by CoffeeScript 1.6.3
var Piechart_ColumnView;

Piechart_ColumnView = (function() {
  Piechart_ColumnView.prototype.animationPriority = 1007;

  function Piechart_ColumnView(chart) {
    this.chart = chart;
    this.scene = this.chart.scene;
    this.renderer = new Piechart_ColumnRenderer(chart, this);
    this.animator = new Piechart_Animations(chart);
    this.layout = new Piechart_ColumnLayout(chart, this);
    this.builder = new Piechart_ColumnBuilder(chart, this);
  }

  Piechart_ColumnView.prototype.updatePie = function(pie) {
    return this.builder.updatePie(pie);
  };

  Piechart_ColumnView.prototype.scrollForward = function(pie, offset) {
    return this.builder.scrollForward(pie, offset);
  };

  Piechart_ColumnView.prototype.scrollBackward = function(pie, offset) {
    return this.builder.scrollBackward(pie, offset);
  };

  Piechart_ColumnView.prototype.onSceneChange = function(event) {};

  Piechart_ColumnView.prototype.doAnimations = function(event) {
    var changedPies, pie, pieChanges, _i, _len, _results;
    if (event.changes.settings || event.changes.pie || event.changes.bounds) {
      pieChanges = false;
      pie = this.scene.peek();
      if (pie) {
        pieChanges || (pieChanges = this.layout.placeColumns(pie));
        if (pieChanges) {
          event.changes.pie = true;
        }
      }
    ...
    changedPies = this.animator.doAnimations(event);
    this.renderer.doAnimations(event);
    _results = [];
    for (_i = 0, _len = changedPies.length; _i < _len; _i++) {
      pie = changedPies[_i];
      _results.push(this.layout.placeLabels(pie, this.renderer.valueAxis["default"], event.context));
    }
    return _results;
  };

  Piechart_ColumnView.prototype.paintScene = function(event) {
    return this.renderer.paintScene(event);
  };

  Piechart_ColumnView.prototype.getMovement = function(pie, event) {
    var bottom, dfraction, doffset, x0, x1, y0, y1;
    bottom = this.scene.y0 + this.scene.height;
    x0 = event.x - event.dx - this.scene.x0;
    y0 = bottom - (event.y - event.dy);
    x1 = event.x - this.scene.x0;
    y1 = bottom - event.y;
    dfraction = (x1 - x0) / this.scene.width / (pie.currentEndAngle - pie.currentStartAngle);
    doffset = y1 - y0;
    return [dfraction, doffset];
  };

  Piechart_ColumnView.prototype.findSliceAt = function(px, py, tolerance) {
    var index, pie, tradd, trmul, _i, _len, _ref, _ref1;
    if ((px < this.scene.x0) || (px > (this.scene.x0 + this.scene.width)) || (py < this.scene.y0) || (py > (this.scene.y0 + this.scene.height))) {
      return null;
    }
    _ref = this.scene.pies;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      _ref1 = this.getColumnTransform(pie), tradd = _ref1[0], trmul = _ref1[1];
      index = Math.floor((px - tradd) / trmul);
      if (index >= 0 && index < pie.allSlices.length) {
        return pie.allSlices[index];
      }
    }
    return this.findLabelAt(px, py, tolerance)[0];
  };

  Piechart_ColumnView.prototype.findPieAt = function(px, py, tolerance) {
    if ((px < this.scene.x0) || (px > (this.scene.x0 + this.scene.width)) || (py < this.scene.y0) || (py > (this.scene.y0 + this.scene.height))) {
      return null;
    }
    return this.scene.peek();
  };

  Piechart_ColumnView.prototype.findLabelAt = function(x, y, tolerance) {
    var index, l, pie, slice, tradd, trmul, x0, y0, _i, _j, _len, _len1, _ref, _ref1, _ref2;
    _ref = this.scene.pies;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      pie = _ref[_i];
      if (pie.background) {
      ...
      _ref1 = this.getColumnTransform(pie), tradd = _ref1[0], trmul = _ref1[1];
      y0 = this.scene.y0 + this.scene.height;
      _ref2 = pie.allSlices;
      for (index = _j = 0, _len1 = _ref2.length; _j < _len1; index = ++_j) {
        slice = _ref2[index];
        x0 = tradd + trmul * index;
        if (slice.currentLabel) {
          l = slice.currentLabel;
          if (l.visible && x0 + l.x - l.hwidth <= x && x0 + l.x + l.hwidth >= x && y0 + l.y - l.hheight <= y && y0 + l.y + l.hheight >= y) {
            return [slice, l];
          }
        }
        if (slice.currentInsideLabel) {
  ...
  Piechart_ColumnView.prototype.getColumnTransform = function(pie) {
    var add, dangle, mul, x0, x1;
    dangle = pie.currentEndAngle - pie.currentStartAngle;
    x0 = this.scene.x0 + this.scene.width * (pie.currentStartAngle + pie.scrollOffset * dangle);
    x1 = this.scene.x0 + this.scene.width * (pie.currentEndAngle + pie.scrollOffset * dangle);
    add = x0;
    mul = (x1 - x0) / pie.columnsInScreen;
    return [add, mul];
  };

  return Piechart_ColumnView;

})();

/*
//@ sourceMappingURL=ColumnView.map
*/
...
Piechart_Impl = (function(_super) {
  ...
  function Piechart_Impl(settings, api) {
    ...
    Piechart_Impl.__super__.constructor.call(this, settings);
    ...
    this.title = this.events.addElement(new Base_Title(this));
  ...
  Piechart_Impl.prototype.EVENT_NAMES = {
    ...
    "pieChange": "onPieChange",
    "pieReadyStateChanged": "onPieReadyStateChanged"
  };
  ...
  Piechart_Impl.prototype.getView = function() {
    if (this.settings.area.type === "pie") {
      return new Piechart_PieView(this);
    } else if (this.settings.area.type === "columns") {
      return new Piechart_ColumnView(this);
    }
  };
  ...
  Piechart_Impl.prototype.onSizeChanged = function(clientWidth, clientHeight) {
    var bottomMargin, height, leftMargin, leftPadding, rightMargin, rightPadding, th, topMargin, tw, width, _ref, _ref1;
    leftMargin = 0;
    rightMargin = 0;
    bottomMargin = 0;
    topMargin = 0;
    if (this.settings.area.type === "columns") {
      _ref = Linearchart_ValueAxis.computeMargins(this.settings.getValueAxisList()), leftMargin = _ref[0], rightMargin = _ref[1], leftPadding = _ref[2], rightPadding = _ref[3];
      this.scene.leftValueAxisSize = leftMargin + leftPadding;
      this.scene.rightValueAxisSize = rightMargin + rightPadding;
      bottomMargin = this.settings.columns.labelsHeight;
    }
    _ref1 = this.title.computeSize(this.layers.canvas.getContext("2d")), tw = _ref1[0], th = _ref1[1];
    topMargin = th;
    height = clientHeight;
    width = clientWidth;
  ...
  Piechart_Impl.prototype.setPie = function(root, offset, animate) {
    if (animate == null) {
      animate = true;
    }
    this.log("Set pie");
    return this.navigator.setPie(root, offset, "api");
  };
  ...
  Piechart_Impl.prototype.notifySelectionChanged = function(origin) {
    return this.dispatchEventParams("selectionChange", this.extendEventParams({
      origin: origin
    }), null);
  };

  Piechart_Impl.prototype.notifyHoverChanged = function(e) {
    return this.dispatchEventParams("hoverChange", this.extendEventParams(e), null);
  };

  Piechart_Impl.prototype.notifyDoubleClick = function(e) {
    return this.dispatchEvent("doubleClick", this.extendEventParams(e), this.defaultDoubleClick);
  };

  Piechart_Impl.prototype.notifyClick = function(e) {
    return this.dispatchEvent("click", this.extendEventParams(e), this.defaultClick);
  };

  Piechart_Impl.prototype.notifyPieChange = function(newPie, origin) {
    return this.dispatchEventParams("pieChange", this.extendEventParams({
      pie: newPie,
      origin: origin
    }), null);
  };

  Piechart_Impl.prototype.notifyPieReady = function(pie, origin) {
